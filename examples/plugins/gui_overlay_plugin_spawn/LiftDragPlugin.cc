...
/////////////////////////////////////////////////
bool LiftDragPlugin::FindJoint(const std::string &_sdfParam, sdf::ElementPtr _sdf,
    physics::JointPtr &_joint)
{
  // Read the required plugin parameters.
  if (!_sdf->HasElement(_sdfParam))
  {
    gzwarn << "Unable to find the <" << _sdfParam << "> parameter." << std::endl;
    return false;
  }
...
void LiftDragPlugin::OnUpdate()
{
  ...
  if(this->HasPropellerWind)
  {
    ignition::math::Vector3d W_P=ignition::math::Vector3d(0, 0, 0);
    // pose of body of propeller_link
    ignition::math::Pose3d pose_propeller = this->propeller_link->WorldPose();
    double propellerRad = this->propeller_joint->GetVelocity(0);
    double motor_constant_ = 5.84e-04;
    double T_propeller = std::abs(propellerRad) * std::abs(propellerRad) * motor_constant_;// Not allowed to have negative thrust.
    double wind_by_propeller = sqrt(T_propeller / (2 * rho * 0.1 * (1 - 0.29))) ;
  ...
  ignition::math::Vector3d velInLDPlane = vel - vel.Dot(spanwiseI)*spanwiseI + W_PI;
